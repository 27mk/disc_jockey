...
def generate_images_for_album(album_id, tracklist):
    print(f"Generating images for album ID: {album_id}")
    try:
        ...
        if not os.path.exists(sticker_path):
            print(f"Album front image {album_image_path} not found, skipping.")
            return

        for side_name, tracks in tracklist.items():
            output_path = f"static/{album_id}_{side_name.replace(' ', '_')}.png"
            print(f"About to generate image for {side_name} and save it to {output_path}")
            ...
            # CHANGE THIS FROM BEING RANDOMIZED TO BEING BASED OF JSON DATA
            ...
            if os.path.exists(output_path):
                print(f"Image saved successfully for album {album_id}, {side_name} at {output_path}")
            else:
                print(f"Failed to save image for album {album_id}, {side_name} at {output_path}")
...
def process_album_data(album_data):
    for album in album_data:
        ...
        generate_images_for_album(album_id, tracklist)
